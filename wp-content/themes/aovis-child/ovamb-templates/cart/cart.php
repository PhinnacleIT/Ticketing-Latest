...
$is_fee_exempt_user = function_exists( 'aovis_mb_is_fee_exempt_user' ) && aovis_mb_is_fee_exempt_user();
...
if ( $is_fee_exempt_user ) {
    $tax_fee          = 0;
    $ticket_fee       = 0;
    $incl_tax         = 'no';
    $enable_tax       = 'no';
    $enable_ticket_fee = 'no';
}
...
<?php else: ?>
    ...
    <article id="mb_cart" class="cart_detail">
        <input
            ...
            data-extra-service="<?php echo esc_attr( json_encode( $extra_service ) ); ?>"
            data-person-type="<?php echo esc_attr( json_encode( $person_type ) ); ?>"
